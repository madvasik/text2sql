"""
Модуль для оценки модели на датасете KaggleDBQA.
"""
...
from .kaggledbqa_dataset import KaggleDBQADataset, KaggleDBQAExample
...
class KaggleDBQAEvaluator:
    """Класс для оценки модели на датасете KaggleDBQA."""
    
    def __init__(
        self,
        dataset: KaggleDBQADataset,
    ...
    def evaluate(
        ...
        """
        examples = self.dataset.load_examples(split=split, mode=mode)
    ...
    def _evaluate_single(self, example: KaggleDBQAExample) -> EvaluationResult:
        """
        ...
        # Формируем описание схемы с документацией если доступно
        schema_description = None
        if self.use_documentation and example.db_documentation:
            schema_description = example.db_documentation
